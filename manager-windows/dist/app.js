...
function logo() { return '<span class="brandmark" aria-label="NarrowVision"><svg viewBox="0 0 108 108" aria-hidden="true"><defs><linearGradient id="nv-logo-gradient" x1="18" y1="20" x2="90" y2="88" gradientUnits="userSpaceOnUse"><stop stop-color="#557DFF"/><stop offset=".52" stop-color="#7B35FF"/><stop offset="1" stop-color="#D347F5"/></linearGradient></defs><path fill="url(#nv-logo-gradient)" d="M21 23c0-4 4.6-6.3 7.8-3.9L68 48.5V33.8c0-1.8 1-3.4 2.6-4.2l14.5-7.2c3.2-1.6 6.9.7 6.9 4.2v58c0 4-4.6 6.3-7.8 3.9L45 59.1v15.1c0 1.8-1 3.4-2.6 4.2l-14.5 7.2c-3.2 1.6-6.9-.7-6.9-4.2V23Zm0 0 71 53.3V59.1L21 23Z"/></svg></span>'; }
...
async function resolveIdentity() {
  ...
  if(manager!==true) throw new Error("Dit account heeft geen toegang tot NarrowVision Admin.");
  let profile=null;
...
function renderDashboard() {
  ...
  document.querySelectorAll("[data-page]").forEach(b=>b.onclick=async()=>{activePage=b.dataset.page;query="";error="";await loadOperationalData();if(activePage==="requests")await markManagerRequestsViewed();renderDashboard()});
...
function customerRequestsPage(ownRequests){
  return `<section class="panel"><div class="panel-head"><div><h2>Mijn verzoeken</h2><p>Stuur een vraag of probleem rechtstreeks naar NarrowVision.</p></div><button class="primary" id="new-request">＋ Nieuw verzoek</button></div>${ticketFolderBar(ownRequests)}${customerRequestsHtml(filterTickets(ownRequests))}</section>`;
}
function renderCustomerDashboard(){
  ...
  const active=licenseState(customer).allowed;
  ...
  app.innerHTML=`<main class="app-shell customer-shell"><aside class="sidebar"><div class="brand">${logo()}<span>NARROWVISION<small>ADMIN</small></span></div><nav>${customerNav("overview","▦","Overzicht")}${customerNav("players","▰","Players")}${customerNav("media","▧","Media")}${customerNav("playlists","▶","Afspeellijsten")}${customerNav("planning","≡","Planning")}${customerNav("requests","✉","Verzoeken",unreadCustomerCount())}</nav><button class="logout" id="logout">↪ Uitloggen</button><div class="side-foot"><span class="shield">✓</span><div><strong>Klantaccount</strong><span>${esc(session?.user?.email||"")}</span></div></div></aside><section class="workspace"><header><div><p class="eyebrow">NARROWVISION ADMIN · ${esc(pageTitle.toUpperCase())}</p><h1>${heading}</h1><p>${esc(title)} · ${esc(customer?.customerNumber||"")}</p></div></header>${error?`<p class="error-banner customer-error">${esc(error)}</p>`:""}${content}</section></main><div id="modal-root"></div>`;
  ...
  document.querySelectorAll("[data-customer-page]").forEach(button=>button.onclick=async()=>{customerPage=button.dataset.customerPage;error="";await loadOperationalData();if(customerPage==="requests")await markCustomerRequestsViewed();renderCustomerDashboard()});
...
function renderAdminAccessModal(c){
  document.getElementById("modal-root").innerHTML=`<div class="modal-bg" id="modal-bg"><form class="modal" id="admin-access-form"><div class="modal-title"><div><p class="eyebrow">EENMALIGE TOEGANG</p><h2>Adminaccount voor ${esc(c.name)}</h2></div><button type="button" id="close">×</button></div><p class="modal-copy">NarrowVision maakt een sterk tijdelijk wachtwoord. Het wordt na aanmaak één keer getoond en moet bij de eerste login direct worden gewijzigd.</p><label>Naam<input id="admin-name" required value="${esc(c.contactName)}"></label><label>E-mailadres<input id="admin-email" required type="email" value="${esc(c.email)}"></label><p class="login-error hidden" id="modal-error"></p><div class="modal-actions"><button type="button" id="cancel">Annuleren</button><button class="primary" id="admin-create-button">Toegang genereren</button></div></form></div>`;
  const close=()=>document.getElementById("modal-root").innerHTML="";
...
function playersPanel(){
  ...
  return `<section class="panel"><div class="panel-head"><div><h2>Players</h2><p>Status, versie en bediening op afstand.</p></div><label class="search">⌕<input id="search" value="${esc(query)}" placeholder="Zoek player"></label></div><div class="table-wrap"><table><thead><tr><th>Player</th><th>Klant</th><th>Status</th><th>Versie</th><th>Acties</th></tr></thead><tbody>${shown.length?shown.map(playerRow).join(""):'<tr><td colspan="5" class="empty">Nog geen players geregistreerd. Open NarrowVision Player om een koppelcode te krijgen.</td></tr>'}</tbody></table></div></section>`;
}
