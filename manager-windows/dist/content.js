...
function renderScheduleModal(){
  document.getElementById("modal-root").innerHTML=`<div class="modal-bg" id="modal-bg"><form class="modal" id="schedule-form"><div class="modal-title"><div><p class="eyebrow">NIEUWE PLANNING</p><h2>Wanneer moet hij spelen?</h2></div><button type="button" id="close">×</button></div><label>Naam<input id="schedule-name" maxlength="120" required placeholder="Bijvoorbeeld: Openingstijden"></label><label>Afspeellijst<select id="schedule-playlist">${playlists.map(p=>`<option value="${esc(p.id)}">${esc(p.name)}</option>`).join("")}</select></label><fieldset class="weekday-field"><legend>Dagen</legend><div class="weekday-grid">${DAY_NAMES.map((name,day)=>`<label><input type="checkbox" name="schedule-day" value="${day}" ${day>=1&&day<=5?"checked":""}><span>${name}</span></label>`).join("")}</div></fieldset><div class="form-row"><label>Begintijd<input id="schedule-start" type="time" value="08:00" required></label><label>Eindtijd<input id="schedule-end" type="time" value="18:00" required></label></div><p class="login-error hidden" id="modal-error"></p><div class="modal-actions"><button type="button" id="cancel">Annuleren</button><button class="primary" id="schedule-create">Planning opslaan</button></div></form></div>`;
  modalCloseBindings();document.getElementById("schedule-form").onsubmit=createSchedule;
}
async function createSchedule(e){
  e.preventDefault();const button=document.getElementById("schedule-create");button.disabled=true;
  try{
    const days=[...document.querySelectorAll('[name="schedule-day"]:checked')].map(x=>Number(x.value));
    if(!days.length)throw new Error("Kies minimaal één dag.");
    const start=document.getElementById("schedule-start").value,end=document.getElementById("schedule-end").value;
    if(start>=end)throw new Error("De eindtijd moet later zijn dan de begintijd.");
    await request("/rest/v1/content_schedules",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({organization_id:identity.organizationId,playlist_id:document.getElementById("schedule-playlist").value,created_by:session.user.id,name:document.getElementById("schedule-name").value.trim(),days_of_week:days,start_time:start,end_time:end,active:true,timezone:"Europe/Amsterdam"})});
    document.getElementById("modal-root").innerHTML="";await loadContentSchedules();renderCustomerDashboard();
