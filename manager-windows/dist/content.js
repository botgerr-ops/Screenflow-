...
function bindContentPage(){
  ...
  document.getElementById("new-schedule")?.addEventListener("click",()=>renderScheduleModal(null));
  document.querySelectorAll("[data-edit-schedule]").forEach(b=>b.onclick=()=>renderScheduleModal(b.dataset.editSchedule));
...
function renderScheduleModal(existingId=null){
 const existing=existingId==null?null:contentSchedules.find(s=>s.id===existingId);
 if(existingId!=null&&(!UUID_PATTERN.test(existingId)||!existing)){error="Planning kan niet worden geopend: ongeldige of verouderde planning-ID. Vernieuw de pagina en probeer opnieuw.";renderCustomerDashboard();return}
 ...
 modalCloseBindings();document.getElementById("schedule-form").onsubmit=e=>saveSchedule(e,existingId);
}
async function saveSchedule(e,existingId){
 e.preventDefault();const button=document.getElementById("schedule-create");button.disabled=true;
 try{const days=[...document.querySelectorAll('[name="schedule-day"]:checked')].map(x=>Number(x.value));const start=document.getElementById("schedule-start").value,end=document.getElementById("schedule-end").value,active=document.getElementById("schedule-active").checked;if(existingId!=null&&!UUID_PATTERN.test(existingId))throw new Error("De planning-ID is ongeldig. Vernieuw de pagina en probeer opnieuw.");if(!days.length)throw new Error("Kies minimaal één dag.");if(start>=end)throw new Error("De eindtijd moet later zijn dan de begintijd.");const value={playlist_id:document.getElementById("schedule-playlist").value,name:document.getElementById("schedule-name").value.trim(),days_of_week:days,start_time:start,end_time:end,active,timezone:"Europe/Amsterdam"};const conflict=scheduleConflict({...value,days},existingId);if(conflict)throw new Error("Planningconflict: "+(conflict.playlists?.name||"een andere playlist")+" overlapt deze periode.");if(existingId)await request("/rest/v1/content_schedules?id=eq."+encodeURIComponent(existingId),{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({...value,updated_at:new Date().toISOString()})});else await request("/rest/v1/content_schedules",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({...value,organization_id:identity.organizationId,created_by:session.user.id})});document.getElementById("modal-root").innerHTML="";await loadContentSchedules();renderCustomerDashboard();
